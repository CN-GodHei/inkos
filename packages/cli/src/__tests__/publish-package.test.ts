...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "@cn-godhei/inkos-core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
          {
            name: "@cn-godhei/inkos",
            version: "0.4.6",
            dependencies: {
              "@cn-godhei/inkos-core": "workspace:*",
              commander: "^13.0.0",
            },
          },
          null,
      ...
      expect(cliPackageJson.dependencies["@cn-godhei/inkos-core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("links internal source dependencies through the workspace", async () => {
    ...
    expect(cliPackageJson.dependencies["@cn-godhei/inkos-core"]).toBe("workspace:*");
    expect(cliPackageJson.dependencies["@cn-godhei/inkos-studio"]).toBe("workspace:*");
    expect(studioPackageJson.dependencies["@cn-godhei/inkos-core"]).toBe("workspace:*");
  });
  ...
  it("accepts canonical workspace protocol in source manifests", async () => {
    ...
    try {
      ...
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@cn-godhei/inkos-core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
        ...
        `${JSON.stringify(
          {
            ...
            version: "0.5.1",
            dependencies: {
              "@cn-godhei/inkos-core": "workspace:0.5.0",
            },
          },
          null,
          2,
  ...
  it("replaces workspace dependencies before npm pack", { timeout: CLI_PACK_TEST_TIMEOUT_MS }, async () => {
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@cn-godhei/inkos-core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["@cn-godhei/inkos-studio"]).toBe(studioPackageJson.version);
    } finally {
