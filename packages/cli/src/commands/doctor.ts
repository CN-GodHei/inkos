...
import { fetchWithProxy } from "@cn-godhei/inkos-core";
import {
...
export const doctorCommand = new Command("doctor")
  ...
  .action(async (opts: { repairNodeRuntime?: boolean }) => {
    ...
    // 5. Check effective LLM config (Studio project base + env/CLI overlay, or legacy env)
    {
      ...
      const { isApiKeyOptionalForEndpoint } = await import("@cn-godhei/inkos-core");
    ...
    try {
      const { StateManager } = await import("@cn-godhei/inkos-core");
    ...
    try {
      const { createLLMClient, chatCompletion, LLMConfigSchema, isApiKeyOptionalForEndpoint, resolveServiceModelsBaseUrl } = await import("@cn-godhei/inkos-core");
